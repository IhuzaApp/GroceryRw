...
const UPDATE_DELIVERY_ISSUE = gql`
  mutation UpdateDeliveryIssuePriority($code: Int!, $priority: String!) {
    update_Delivery_Issues(where: {code: {_eq: $code}}, _set: {priority: $priority}) {
      affected_rows
    }
  }
`;
...
const UPDATE_TICKET = gql`
  mutation UpdateTicketPriority($code: Int!, $priority: String!) {
    update_tickets(where: {ticket_num: {_eq: $code}}, _set: {priority: $priority}) {
      affected_rows
    }
  }
`;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!hasuraClient) return res.status(500).json({ error: "Missing DB client" });

  try {
    const data = await hasuraClient.request<any>(GET_ISSUE_OR_TICKET, { code: codeNum });

    const isDeliveryIssue = data.Delivery_Issues?.length > 0;
    const isTicket = data.tickets?.length > 0;

    if (!isDeliveryIssue && !isTicket) {
      return res.status(404).json({ error: "Could not find any issue or ticket with that tracking code." });
    }

    if (isDeliveryIssue) {
      await hasuraClient.request(UPDATE_DELIVERY_ISSUE, { code: codeNum, priority: urgency });
    } else {
      await hasuraClient.request(UPDATE_TICKET, { code: codeNum, priority: urgency });
    }

    const urgencyIndicator = urgency === "high" ? "🔴 HIGH" : urgency === "medium" ? "🟡 MEDIUM" : "🟢 LOW";
    ...
    await notifySystemToSlack({
      ...
      message: `${session.user.name || "A user"} is following up on ${typeLabel} #${codeNum}.\n*Urgency*: ${urgencyIndicator}\n*Message*: ${message}`,
      context: {
    ...
    return res.status(200).json({ success: true, message: "Priority updated and support notified." });
  } catch (error) {
