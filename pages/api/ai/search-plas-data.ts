...
const SEARCH_ALL_PRODUCTS = gql`
  query SearchAllProducts($keyword: String!, $shopKeyword: String!) {
    Products(
      where: {
        is_active: { _eq: true }
        ProductName: { name: { _ilike: $keyword } }
        Shop: { name: { _ilike: $shopKeyword } }
      }
    ) {
      id
      final_price
    ...
    restaurant_menu(
      where: {
        is_active: { _eq: true }
        dishes: { name: { _ilike: $keyword } }
        Restaurants: { name: { _ilike: $shopKeyword } }
      }
    ) {
      price
      Restaurants {
    ...
    business_stores(
      where: { is_active: { _eq: true }, name: { _ilike: $shopKeyword } }
    ) {
      id
      name
      latitude
...
const SEARCH_ALL_STORES = gql`
  query SearchAllStores($keyword: String!) {
    Shops(
      where: { is_active: { _eq: true }, name: { _ilike: $keyword } }
      limit: 40
    ) {
      id
      name
...
export default async function handler(
  ...
  try {
    ...
    if (action === "search_products") {
      ...
      console.log(
        `[AI Search API] Searching products keyword: ${keyword}, shop: ${shopKeyword}, maxPrice: ${maxPrice}`
      );

      const result = await hasuraClient.request<any>(SEARCH_ALL_PRODUCTS, {
        keyword,
        shopKeyword,
      });
      ...
      console.log(
        `[AI Search API] Combined raw products matched: ${formattedProducts.length}`
      );
      ...
      if (maxPrice) {
        ...
        console.log(
          `[AI Search API] After budget filter (<= ${maxPrice}): ${formattedProducts.length}`
        );
      }
      ...
      console.log(
        `[AI Search API] Returning ${finalResults.length} sanitized items to AI`
      );
      return res.status(200).json({ results: finalResults });
    } else if (action === "search_stores") {
      const keyword = `%${params.keyword || ""}%`;
      console.log(
        `[AI Search API] Searching ALL stores with keyword: ${keyword}`
      );

      const result = await hasuraClient.request<any>(SEARCH_ALL_STORES, {
        keyword,
      });
      ...
      (result.Restaurants || []).forEach((r: any) =>
        allStores.push({
          type: "Restaurant",
          id: r.id,
          name: r.name,
      ...
      (result.business_stores || []).forEach((bs: any) =>
        allStores.push({
          type: "Business",
          id: bs.id,
          name: bs.name,
      ...
      console.log(
        `[AI Search API] All Stores matched: ${sanitizedStores.length}`
      );
      ...
      const result = await hasuraClient.request<any>(GET_USER_CARTS, {
        user_id,
      });
      const formattedCarts = (result.Carts || []).map((c: any) => {
        const subtotal = c.Cart_Items.reduce(
          (sum: number, i: any) => sum + parseFloat(i.price) * i.quantity,
          0
        );
        return {
          id: c.id,
      ...
      const [cartData, configData] = await Promise.all([
        hasuraClient.request<any>(GET_CART_WITH_ITEMS, {
          user_id,
          shop_id: shop_id || params.shopId,
        }),
      ...
      if (address_id) {
        const addrRes = await hasuraClient.request<any>(
          gql`
            query ($id: uuid!) {
        ...
        const addrRes = await hasuraClient.request<any>(GET_ADDRESSES, {
          user_id,
        });
        selectedAddr =
          addrRes.Addresses.find((a: any) => a.is_default) ||
          addrRes.Addresses[0];
      }
      ...
      const units = cart.Cart_Items.reduce(
      ...
      const unitsSurcharge =
        extraUnits * (config ? parseInt(config.unitsSurcharge) : 0);
      ...
      if (shopLat && shopLon && userLat && userLon) {
        ...
        const distanceSurcharge =
          ...
          (config ? parseInt(config.distanceSurcharge) : 0);
        const rawDistanceFee = baseDeliveryFee + distanceSurcharge;
        const cappedDistanceFee = config
          ? parseInt(config.cappedDistanceFee)
          : 0;
      ...
      return res.status(200).json({
        ...
        pricing_token: Buffer.from(
          JSON.stringify({ subtotal, total, shop_id: shop_id || params.shopId })
        ).toString("base64"),
      });
    }
