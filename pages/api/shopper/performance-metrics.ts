...
const calculateDeliveryTimePerformance = (orders: any[]): number => {
  ...
  const onTimeDeliveries = deliveredOrders.filter(order => {
    try {
      ...
      logger.error("Error calculating delivery time", "PerformanceMetrics", { error, orderId: order.id });
      return false;
    }
  });
...
const calculateCombinedDeliveryTimePerformance = (regularOrders: any[], reelOrders: any[]): number => {
  // For regular orders, use delivery_time vs updated_at
  const deliveredRegularOrders = regularOrders.filter(order => 
    order.status === 'delivered' && 
    order.delivery_photo_url && 
    order.delivery_time
  );
  ...
  const deliveredReelOrders = reelOrders.filter(order => 
    order.status === 'delivered' && 
    order.delivery_photo_url
  );
  ...
  const onTimeRegularDeliveries = deliveredRegularOrders.filter(order => {
    try {
      ...
      logger.error("Error calculating regular order delivery time", "PerformanceMetrics", { error, orderId: order.id });
      return false;
    }
  });
...
const calculateResponseTime = (orders: any[]): number => {
  ...
  const responseTimes = acceptedOrders.map(order => {
    try {
      ...
      logger.error("Error calculating response time", "PerformanceMetrics", { error, orderId: order.id });
      return 0;
    }
  });
...
const calculateOverallScore = (metrics: Partial<PerformanceMetrics>): number => {
  ...
  const totalScore = customerRatingScore + onTimeDeliveryScore + orderAccuracyScore + acceptanceRateScore;
...
export default async function handler(
  ...
  try {
    ...
    const orderAccuracy = assignedOrders > 0 
      ? Math.round((completedOrdersWithPhotos / assignedOrders) * 100)
      : 100;
