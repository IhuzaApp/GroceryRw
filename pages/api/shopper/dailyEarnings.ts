...
const formatEarningsData = (orders: OrdersResponse['Orders'], period: string) => {
  ...
  // For 'this-month' or 'last-month', we show weekly data
    ...
    const firstOrderDate = orders.length > 0 ? new Date(orders[0].updated_at) : new Date();
    const startOfMonth = new Date(firstOrderDate.getFullYear(), firstOrderDate.getMonth(), 1);
...
export default async function handler(
  ...
  try {
    ...
    const data = await hasuraClient.request<OrdersResponse>(GET_DAILY_EARNINGS, {
