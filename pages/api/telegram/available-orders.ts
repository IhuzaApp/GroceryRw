...
// GraphQL query to fetch available orders older than 40 minutes
const GET_AVAILABLE_ORDERS = gql`
  query GetAvailableOrders($forty_minutes_ago: timestamptz!) {
    Orders(
      where: { 
        shopper_id: { _is_null: true }, 
        status: { _eq: "PENDING" },
        created_at: { _lt: $forty_minutes_ago }
      }
...
export default async function handler(
  ...
  try {
    ...
    const data = await hasuraClient.request<GraphQLResponse>(
      ...
      {
        forty_minutes_ago: fortyMinutesAgo.toISOString(),
      }
    );
