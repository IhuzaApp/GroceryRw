...
const GET_ORDER_BY_PIN = gql`
  query GetOrderByPin($pin: String!) {
    Orders(where: { pin: { _eq: $pin } }) {
      id
      pin
      OrderID
    }
    package_delivery(where: { DeliveryCode: { _eq: $pin } }) {
      id
      DeliveryCode
    }
    restaurant_orders(where: { pin: { _eq: $pin } }) {
      id
      OrderID
      pin
    }
    reel_orders(where: { pin: { _eq: $pin } }) {
      id
      OrderID
      pin
    }
    businessProductOrders(where: { pin: { _eq: $pin } }) {
      id
      OrderID
      pin
    }
...
export default async function handler(
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (!pin || !order_source)
        return res.status(400).json({ error: "Missing pin or order_source" });

      // Look up order ID by pin
      const ordersRes = await hasuraClient.request<any>(GET_ORDER_BY_PIN, {
        pin,
      });
      ...
      if (order_source.toLowerCase() === "shop" && ordersRes.Orders?.length) {
        ...
        order_source.toLowerCase() === "restaurant" &&
        ordersRes.restaurant_orders?.length
      ) {
        order_id = ordersRes.restaurant_orders[0].id;
        found = true;
      } else if (
        order_source.toLowerCase() === "reel" &&
        ordersRes.reel_orders?.length
      ) {
        reel_order_id = ordersRes.reel_orders[0].id;
        found = true;
      } else if (
        order_source.toLowerCase() === "business" &&
        ordersRes.businessProductOrders?.length
      ) {
        business_order_id = ordersRes.businessProductOrders[0].id;
        found = true;
      } else if (
        order_source.toLowerCase() === "package" &&
        ordersRes.package_delivery?.length
      ) {
      ...
      try {
        await sendSupportTicketToSlack({
          orderId: String(
            order_id || package_id || business_order_id || reel_order_id || pin
          ),
          orderDisplayId: pin,
          orderType:
            order_source.toLowerCase() === "shop"
              ? "regular"
              : order_source.toLowerCase() === "business"
              ? "business"
              : order_source.toLowerCase() === "restaurant"
    ...
    res
      .status(500)
      .json({ error: "Failed to process delivery issues request" });
  }
}
