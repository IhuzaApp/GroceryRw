...
export default async function handler(
  ...
  try {
    ...
    const allDelayed: any[] = [
      ...
      ...(data.restaurant_orders || []).map((o: any) => ({
        ...o,
        type: "restaurant",
      })),
    ];
    ...
    console.log(
      `🔍 [Delayed Orders Cron] Found ${allDelayed.length} delayed orders.`
    );
    ...
    for (const order of allDelayed) {
      ...
      const orderNumber =
        order.OrderID != null ? String(order.OrderID).padStart(4, "0") : "—";
      ...
      if (customerId) {
        await sendDelayedOrderNotification(
          customerId,
          orderId,
          minutesDelayed,
          orderNumber
      ...
      .map((o) => {
        const orderNumber =
          o.OrderID != null
            ...
            : o.id.substring(0, 8);
        const store =
          o.Shop?.name || o.Reel?.title || o.Restaurant?.name || "Unknown";
        const delay = Math.round(
          (Date.now() - new Date(o.delivery_time).getTime()) / 60000
        );
