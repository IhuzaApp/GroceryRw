...
export default async function handler(
  ...
  try {
    ...
    const validationResults = {
      ...
      customLocationRule: !currentSettings.use_live_location || currentSettings.custom_locations?.length === 0,
      // Rule: Maximum 2 custom locations
      maxLocationsRule: (currentSettings.custom_locations?.length || 0) <= 2,
      // Rule: Valid max distance
      validDistance: ["5", "10", "15", "20", "25", "30"].includes(currentSettings.max_distance),
      // Rule: Valid sound settings
      validSoundSettings: currentSettings.sound_settings?.enabled !== undefined && 
    ...
    const totalTests = Object.keys(validationResults).length + Object.keys(typeValidation).length;
    ...
    logger.info("Location management test completed", "LocationManagementTest", {
      user_id: TEST_USER_ID,
    ...
    return res.status(200).json({
      ...
      recommendations: [
        testScore < 100 ? "Some validation rules failed - check settings" : "All validation rules passed",
        currentSettings.use_live_location && currentSettings.custom_locations?.length > 0 
    ...
    logger.error("Error testing location management", "LocationManagementTest", error);
