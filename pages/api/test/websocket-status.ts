...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const status = {
      ...
      connections: global.activeConnections ? Array.from(global.activeConnections.entries()).map(([userId, conn]) => ({
        userId,
        socketId: conn.socketId,
        hasLocation: !!conn.location,
        lastSeen: conn.lastSeen
      })) : [],
      clusters: global.locationClusters ? Array.from(global.locationClusters.entries()).map(([id, cluster]) => ({
        id,
