...
export default async function handler(
  ...
  try {
    ...
    const GET_ACTIVE_AVAILABILITY = gql`
      query GetActiveAvailability($user_id: uuid!, $current_time: timetz!, $current_day: Int!) {
        Shopper_Availability(
          where: {
            _and: [
              { user_id: { _eq: $user_id } }
    ...
    const [allAvailability, currentDayAvailability, activeAvailability] = await Promise.all([
      hasuraClient.request(GET_ALL_SHOPPER_AVAILABILITY, { user_id }) as any,
      hasuraClient.request(GET_CURRENT_DAY_AVAILABILITY, { user_id, current_day: currentDay }) as any,
      hasuraClient.request(GET_ACTIVE_AVAILABILITY, { user_id, current_time: currentTime, current_day: currentDay }) as any
    ]);
    ...
    return res.status(200).json({
      ...
      availability_check: {
        ...
        has_current_day_records: currentDayAvailability.Shopper_Availability.length > 0,
        has_active_records: activeAvailability.Shopper_Availability.length > 0
      }
    });

  } catch (error) {
    logger.error("Error checking shopper availability", "CheckShopperAvailability", error);
