...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const suppliersData: any = await hasuraClient.request(SUPPLIERS_QUERY, { shopId });
    const suppliers: string[] = (suppliersData.suppliers || [])
      .map((s: any) => s.supplier).filter(Boolean);

    // If no search term, just return suppliers
    if (!hasSearchTerm) {
      return res.status(200).json({ found: false, matches: [], productName: null, existingStock: null, suppliers });
    }
    ...
    const LOOKUP_QUERY = gql`
      query LookupProduct($orConditions: [productNames_bool_exp!]!, $stockCondition: Products_bool_exp!) {
    ...
    if (matches.length > 0) console.log("   - Global Matches:", matches.map(m => `${m.name} (SKU: ${m.sku})`));
    if (existingStock) console.log("   - Local Stock ID:", existingStock.id, "SKU:", existingStock.sku);
