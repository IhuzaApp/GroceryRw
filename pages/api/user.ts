...
export default async function handler(
  ...
  try {
    // Fetch user profile
    const userData = await hasuraClient.request<{ Users_by_pk: any }>(GET_CURRENT_USER, { id: user_id });
    const user = userData.Users_by_pk;

    // Fetch order count
    const orderData = await hasuraClient.request<{ Orders_aggregate: { aggregate: { count: number } } }>(GET_ORDER_COUNT, { user_id });
