...
export default function AddStock() {
  ...
  const [scannerContext, setScannerContext] = useState<"SEARCH" | "FORM">(
    "SEARCH"
  );
  ...
  const [selectedProductName, setSelectedProductName] =
    useState<ProductNameRecord | null>(null);
  ...
  const lookupProduct = useCallback(
    async (value: string, isBarcode = false) => {
      ...
      try {
        ...
        if (data.found) {
          const productMatches: ProductNameRecord[] =
            data.matches || (data.productName ? [data.productName] : []);
          ...
          if (exactMatch) {
            selectProductName(
              exactMatch,
              data.existingStock &&
                data.existingStock.productName_id === exactMatch.id
                ? data.existingStock
                : null
            );
    ...
    try {
      ...
      if (step === "TOP_UP" && existingStock) {
        ...
        body = {
          ...
          sku: newSku || selectedProductName.sku,
        };
      } else {
        // NEW_PRODUCT
        if (!newName.trim()) {
  ...
  const inputCls =
    "w-full rounded-2xl border border-gray-200 bg-white px-4 py-3.5 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none transition focus:border-green-500 focus:ring-2 focus:ring-green-500/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500";
  const labelCls =
    "mb-1.5 block text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400";
    ...
    return (
      <div className="relative" onClick={(e) => e.stopPropagation()}>
        ...
        {supplierOpen && (supplier.trim() || filteredSuppliers.length > 0) && (
          <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xl duration-200 animate-in fade-in zoom-in-95 dark:border-gray-700 dark:bg-gray-800">
  ...
  const renderImageSection = () => (
    <div className="mb-6 overflow-hidden rounded-3xl border border-gray-100 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="flex flex-col items-center gap-4">
        {productImage ? (
          <div className="group relative h-40 w-full overflow-hidden rounded-2xl bg-gray-50 dark:bg-black/20">
            <img
              src={productImage}
              alt="Product"
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-gray-100 text-xl font-bold dark:bg-gray-700"
              >
                −
              </button>
              <input
              ...
              <button
                ...
                className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-green-500 text-xl font-bold text-white"
              >
                +
              </button>
            </div>
  ...
  return (
    <div
      className="min-h-screen bg-gray-50 pb-24 dark:bg-black"
      onClick={() => setSupplierOpen(false)}
    >
      <Head>
        <title>Add Stock — Mobile POS</title>
      </Head>
      ...
      <div className="mx-auto max-w-md p-4 pt-6">
        {/* ── SCAN / SEARCH ── */}
        {step === "SCAN" && (
          <div className="space-y-5 duration-300 animate-in fade-in slide-in-from-bottom-4">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Scan barcode or search by product name / SKU
              </p>
            </div>

            <button
              onClick={() => {
                setScannerContext("SEARCH");
                setShowScanner(true);
              }}
            ...
            <form onSubmit={handleManualSearch} className="space-y-3">
              ...
              <button
                type="submit"
                disabled={!searchInput.trim() || looking}
        ...
        {/* ── SELECT MATCH ── */}
        {step === "SELECT_MATCH" && (
          <div className="space-y-4 duration-300 animate-in fade-in slide-in-from-bottom-4">
            ...
            {matches.map((m) => (
              <button
                ...
                onClick={() =>
                  selectProductName(
                    m,
                    existingStock && existingStock.productName_id === m.id
                      ? existingStock
                      : null
                  )
                }
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {m.barcode || m.sku || "No barcode/SKU"}
                  </p>
                </div>
            ...
            <div className="rounded-3xl border border-green-200 bg-green-50 p-5 dark:border-green-500/20 dark:bg-green-500/10">
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-sm">
                ...
                {selectedProductName.barcode && (
                  <span className="rounded-xl bg-white px-3 py-1 font-mono text-xs text-gray-500 dark:bg-gray-800">
                    {selectedProductName.barcode}
                  </span>
                )}
              </div>
        ...
        {step === "ADD_TO_SHOP" && selectedProductName && (
          <div className="space-y-5 duration-300 animate-in fade-in slide-in-from-bottom-4">
            <div className="rounded-3xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-500/20 dark:bg-blue-500/10">
              ...
              {selectedProductName.barcode && (
                <p className="mt-2 font-mono text-xs text-gray-500">
                  Barcode: {selectedProductName.barcode}
                </p>
              )}
            </div>
            {renderImageSection()}
            ...
            <button
              onClick={handleSave}
              disabled={saving || !quantity || !price}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-green-600 py-5 text-lg font-black text-white shadow-xl shadow-green-600/30 transition hover:bg-green-700 disabled:opacity-50"
            >
              {saving ? (
                <Loader2 className="h-5 w-5 animate-spin" />
          ...
          <div className="space-y-5 duration-300 animate-in fade-in slide-in-from-bottom-4">
            <div className="rounded-3xl border border-orange-200 bg-orange-50 p-4 dark:border-orange-500/20 dark:bg-orange-500/10">
              ...
              <p className="mt-0.5 text-sm text-orange-700 dark:text-orange-300">
                Not found in database. Fill in details below.
              </p>
            </div>
            ...
            <button
              onClick={handleSave}
              disabled={saving || !newName.trim() || !quantity}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-green-600 py-5 text-lg font-black text-white shadow-xl shadow-green-600/30 transition hover:bg-green-700 disabled:opacity-50"
            >
              {saving ? (
                <Loader2 className="h-5 w-5 animate-spin" />
        ...
        {/* ── SUCCESS ── */}
        {step === "SUCCESS" && (
          <div className="flex flex-col items-center justify-center py-16 text-center duration-500 animate-in fade-in zoom-in-95">
            ...
            <div className="mt-10 flex w-full flex-col gap-3">
              ...
              <button
                onClick={() => router.push("/MobilePOS/Dashboard")}
