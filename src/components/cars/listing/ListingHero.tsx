...
export default function ListingHero({
  ...
  return (
    <div className="relative h-[200px] w-full md:hidden">
      ...
      <div className="absolute left-0 right-0 top-10 flex items-center justify-between px-6">
        <h1 className="font-outfit text-xl font-black !text-white text-white">
          Explore
        </h1>
        <div className="flex gap-2">
          <button
            onClick={onSearchClick}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-black/20 text-white shadow-lg backdrop-blur-md"
          >
      ...
      <div className="absolute bottom-4 left-6 right-6">
        <div className="flex rounded-[1.25rem] border border-white/10 bg-black/20 p-1 backdrop-blur-xl">
          <button
            onClick={() => setActiveMainTab("explore")}
            className={`flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 font-outfit text-xs font-black transition-all ${
          ...
          >
            Bookings
            {bookingCount > 0 && (
              <span className="ml-1 rounded-full bg-green-500 px-1.5 py-0.5 text-[8px] font-black !text-white text-white">
                {bookingCount}
              </span>
            )}
          </button>
