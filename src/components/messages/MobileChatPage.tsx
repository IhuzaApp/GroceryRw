...
const CustomerMessage: React.FC<{
  ...
  const rawContent = "text" in message ? message.text : (message as Message).text || (message as Message).message || "";
      ...
      <div
        className={`relative max-w-[85%] px-4 py-2 text-[15px] leading-relaxed shadow-sm transition-all ${
          isCurrentUser
            ...
            : "bg-[var(--bg-secondary)] text-[var(--text-primary)] rounded-2xl rounded-tl-sm"
        }`}
      >
        <div className="whitespace-pre-wrap font-normal break-words">{messageContent}</div>
        <div className="flex items-center justify-end gap-1 mt-0.5">
          <span className={`text-[11px] select-none ${isCurrentUser ? "text-green-100" : "text-[var(--text-secondary)]"}`}>
            {formatMessageTime(message.timestamp)}
          </span>
          {isCurrentUser && statusLabel && (
            <span className="text-[11px] font-bold text-green-100 tracking-wide select-none">
              {statusLabel === "Sending..." ? "..." : "✓"}
            </span>
          )}
        </div>
...
export default function MobileChatPage({
  ...
  counterpart: { id: string; name: string; avatar: string; role?: "shopper" | "business" | "customer"; phone?: string };
  onBack: () => void;
}) {
  ...
  useEffect(() => {
    if (!providedConversationId && orderId && session?.user?.id && counterpart?.id) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setPendingMessages((prev) =>
        prev.filter((p) => !msgs.some((m) => m.senderId === p.senderId && (m.text === p.text || m.message === p.text)))
      );
      ...
      snapshot.docs.forEach((d) => {
        const m = d.data();
        if (m.senderId !== session?.user?.id && !m.read) updateDoc(d.ref, { read: true });
      });

      const convRef = doc(db, collectionPath, conversationId);
      getDoc(convRef).then((snap) => {
        if (snap.exists() && snap.data().unreadCount > 0) updateDoc(convRef, { unreadCount: 0 });
      });
  ...
  const displayMessages = React.useMemo(() => {
    const pendingsAsMsg: (Message | PendingMessage)[] = pendingMessages.map((p) => ({ ...p, id: p.tempId, timestamp: p.timestamp }));
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    setPendingMessages((p) => [...p, { tempId, text, senderId: session.user.id, senderType: counterpart.role === "business" ? "business" : "customer", timestamp: new Date() }]);
    setNewMessage("");
    ...
    try {
      ...
      const convRef = doc(db, collectionPath, conversationId);
      await updateDoc(convRef, { lastMessage: text, lastMessageTime: serverTimestamp(), unreadCount: 1 });
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex flex-col bg-[var(--bg-primary)]">
      ...
      <div className="flex-shrink-0 flex items-center justify-between bg-[var(--bg-secondary)] px-2 py-2 shadow-sm z-20 transition-colors">
        <div className="flex items-center flex-1 cursor-pointer" onClick={onBack}>
          <button className="p-2 text-[var(--text-secondary)] hover:bg-black/5 active:bg-black/10 dark:hover:bg-white/5 dark:active:bg-white/10 rounded-full transition-colors mr-1">
            ...
            <div className="relative">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-gray-300 dark:bg-gray-600">
                {counterpart.avatar && counterpart.avatar !== "/images/ProfileImage.png" ? (
                  <img src={counterpart.avatar} alt={counterpart.name} className="h-full w-full object-cover" />
                ) : (
                  <span className="text-lg font-bold text-gray-500 dark:text-gray-300 uppercase">{counterpart.name.charAt(0)}</span>
                )}
              </div>
            </div>
            <div className="flex flex-col flex-1 pl-1">
              <h3 className="text-base font-medium text-[var(--text-primary)] truncate">{counterpart.name}</h3>
              <span className="text-xs text-[var(--text-secondary)] truncate">
                {otherTypingName ? "typing..." : counterpart.role === "business" ? "Business Account" : "Online"}
              </span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1 mx-2">
          {counterpart.phone && (
            <button
              ...
              className="p-2.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-full transition-colors"
            >
          ...
          <button className="p-2.5 text-[var(--text-secondary)] rounded-full">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M12 8a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 6a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 6a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"/></svg>
          </button>
        </div>
      </div>

      <div className="relative flex-1 overflow-y-auto px-3 sm:px-4 py-4 bg-[var(--bg-primary)]">
        {Object.entries(groupedMessages).map(([dateLabel, msgs]) => (
          <div key={dateLabel} className="mb-4">
            <div className="flex justify-center mb-3">
              <span className="bg-[var(--bg-secondary)] text-[var(--text-secondary)] text-xs px-3 py-1 rounded-md shadow-sm uppercase tracking-wide">
                {dateLabel}
              </span>
            </div>
            {msgs.map((message) => (
        ...
        {error && (
          <div className="bg-red-500/10 text-red-400 px-4 py-2 mt-2 rounded-lg text-xs text-center">{error}</div>
        )}
        <div ref={messagesEndRef} className="pb-2" />
      </div>

      {/* Input Box */}
      <div className="flex-shrink-0 bg-[var(--bg-secondary)] p-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] flex items-end gap-2 z-10 transition-colors">
        <button className="p-3 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/></svg>
        </button>
        
        <div className="flex flex-1 items-end bg-[var(--bg-primary)] rounded-[24px]">
          <textarea
            value={newMessage}
        ...
        {newMessage.trim() ? (
          <button onClick={handleSendMessage} className="p-3 ml-1 bg-green-500 text-white rounded-full flex items-center justify-center transition-transform active:scale-95 shadow-sm">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>
          </button>
        ) : (
          <button className="p-3 ml-1 text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-full flex items-center justify-center transition-colors">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M11.999 14.942c2.001 0 3.531-1.53 3.531-3.531V4.35c0-2.001-1.53-3.531-3.531-3.531S8.468 2.349 8.468 4.35v7.061c0 2.001 1.53 3.531 3.531 3.531zm6.238-3.53c0 3.531-2.942 6.002-6.237 6.002s-6.237-2.471-6.237-6.002H3.761c0 4.001 3.178 7.297 7.061 7.885v3.884h2.354v-3.884c3.884-.588 7.061-3.884 7.061-7.885h-2.001z"/></svg>
          </button>
        )}
      </div>
    </div>
