import { Building2, ShieldCheck, Mail, Phone, UploadCloud, Loader2, CheckCircle } from "lucide-react";
...
export default function Step2Identity({ formData, onChange, handleUpload, uploading }: Step2IdentityProps) {
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <div className="absolute left-4 top-1/2 flex -translate-y-1/2 items-center justify-center font-bold text-gray-400">*</div>
            <input
              name="ussd"
              value={formData.ussd}
              onChange={onChange}
        ...
        <div className="space-y-4 md:col-span-2">
          <label className="text-sm font-bold text-gray-600">RDB Certificate (PDF/Image)</label>
          <div className="relative flex flex-col gap-4">
            <div className={`relative flex min-h-[120px] w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed transition-all ${
              formData.rdb_cert_url ? "border-emerald-200 bg-emerald-50/30" : "border-gray-200 bg-gray-50 hover:bg-gray-100"
            }`}>
              {uploading.rdb_cert ? (
                <div className="flex flex-col items-center gap-2">
                  <Loader2 className="h-8 w-8 animate-spin text-[#022C22]" />
                  <span className="text-sm font-medium text-gray-500">Uploading certificate...</span>
                </div>
              ) : formData.rdb_cert_url ? (
                <div className="flex flex-col items-center gap-2 text-emerald-600">
                  <CheckCircle className="h-8 w-8" />
                  <span className="text-sm font-bold">Certificate Uploaded</span>
                  <button 
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-bold text-gray-600">Click to upload RDB Certificate</p>
                    <p className="text-xs text-gray-400">PDF, JPG or PNG (Max 5MB)</p>
                  </div>
                  <input
                    type="file"
                    accept=".pdf,image/*"
                    onChange={(e) => handleUpload(e.target.files?.[0] || null, 'rdb_cert')}
