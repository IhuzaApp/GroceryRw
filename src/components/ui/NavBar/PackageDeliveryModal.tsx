...
export default function PackageDeliveryModal({
  ...
  const fetchWalletBalance = async () => {
    try {
      const res = await authenticatedFetch("/api/queries/personal-wallet-balance");
  ...
  useEffect(() => {
    if (step === 4 && formData.distance > 0) {
      ...
      if (systemConfig) {
        ...
        const baseFee = Number(modeConfig.baseDeliveryFee ?? systemConfig.baseDeliveryFee ?? 1000);
        const surcharge = Number(modeConfig.distanceSurcharge ?? systemConfig.distanceSurcharge ?? 300);
    ...
    try {
      ...
      if (finalPaymentMethod === 'momo') {
        ...
        const pollInterval = setInterval(async () => {
          try {
            const statusRes = await fetch(`/api/momo/request-to-pay-status?referenceId=${momoData.referenceId}`);
  ...
  return (
    <>
      ...
      ` }} />
      <div className="fixed inset-0 z-[100000] flex items-end justify-center p-0 sm:items-center sm:p-4 animate-in fade-in duration-300">
      {/* Backdrop */}
      <div
      ...
      >
        ...
        <div
          className={`flex items-center justify-between px-6 py-6 sm:px-8 ${
            theme === "dark" ? "border-b border-gray-700/50" : "border-b border-gray-100"
          }`}
        >
          <div className="flex items-center gap-4">
            <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${colors.iconBg}`}>
               <svg className={`h-6 w-6 ${colors.iconColor}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M21 8V21H3V8" />
        ...
        {/* Body */}
        <div className={`max-h-[70vh] overflow-y-auto px-6 py-8 sm:px-8 ${theme === "dark" ? "bg-gray-800" : "bg-white"}`}>
          <div className="space-y-6">
            
            {step === 1 && (
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div className="space-y-4">
                   <label className={`block text-base font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Where is it going?</label>
                   <div className="space-y-4">
                      ...
                      {formData.pickup && (
                        <div className="space-y-4 rounded-2xl border border-gray-100 p-4 transition-all dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/20 animate-in fade-in zoom-in-95 duration-500">
                          <div className="flex flex-wrap gap-2">
                            {['Home', 'Office', 'Apartment', 'Hotel'].map((type) => (
                              <button
                                key={type}
                                onClick={() => setFormData({ ...formData, pickupType: type })}
                                className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition-all ${
                                  formData.pickupType === type
                                    ? "bg-green-600 text-white shadow-md shadow-green-900/10"
                                    : theme === "dark" ? "bg-gray-800 text-gray-400 hover:text-white" : "bg-white text-gray-500 hover:text-green-600 border border-gray-100"
                                }`}
                              >
                                {type === 'Home' && <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" strokeWidth={2}/></svg>}
                                {type === 'Office' && <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" strokeWidth={2}/></svg>}
                                {type === 'Apartment' && <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z" strokeWidth={2}/></svg>}
                                {type === 'Hotel' && <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M7 2a1 1 0 00-1 1v1a1 1 0 001 1h10a1 1 0 001-1V3a1 1 0 00-1-1H7zM4 6a1 1 0 00-1 1v14a1 1 0 001 1h16a1 1 0 001-1V7a1 1 0 00-1-1H4zm3 4h2v2H7v-2zm4 0h2v2h-2v-2zm4 0h2v2h-2v-2zm-8 4h2v2H7v-2zm4 0h2v2h-2v-2zm4 0h2v2h-2v-2zm-8 4h2v2H7v-2zm4 0h2v2h-2v-2zm4 0h2v2h-2v-2z" strokeWidth={2}/></svg>}
                                {type}
                              </button>
                            ))}
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3 animate-in fade-in duration-300">
                            {formData.pickupType === 'Home' && (
                              <>
                                <input className={inputStyle} placeholder="Gate Number" value={formData.pickupGateNumber} onChange={(e) => setFormData({...formData, pickupGateNumber: e.target.value})} />
                                <input className={inputStyle} placeholder="Gate Color" value={formData.pickupGateColor} onChange={(e) => setFormData({...formData, pickupGateColor: e.target.value})} />
                              </>
                            )}
                            {formData.pickupType === 'Office' && (
                              <>
                                <input className={inputStyle} placeholder="Office Name" value={formData.pickupOfficeName} onChange={(e) => setFormData({...formData, pickupOfficeName: e.target.value})} />
                                <input className={inputStyle} placeholder="Floor Number" value={formData.pickupFloor} onChange={(e) => setFormData({...formData, pickupFloor: e.target.value})} />
                              </>
                            )}
                            {formData.pickupType === 'Apartment' && (
                              <>
                                <input className={inputStyle} placeholder="Apartment Name" value={formData.pickupApartmentName} onChange={(e) => setFormData({...formData, pickupApartmentName: e.target.value})} />
                                <div className="grid grid-cols-2 gap-2">
                                  <input className={inputStyle} placeholder="Floor" value={formData.pickupFloor} onChange={(e) => setFormData({...formData, pickupFloor: e.target.value})} />
                                  <input className={inputStyle} placeholder="Room" value={formData.pickupRoomNumber} onChange={(e) => setFormData({...formData, pickupRoomNumber: e.target.value})} />
                                </div>
                              </>
                            )}
                            {formData.pickupType === 'Hotel' && (
                              <>
                                <input className={inputStyle} placeholder="Hotel Name" value={formData.pickupHotelName} onChange={(e) => setFormData({...formData, pickupHotelName: e.target.value})} />
                                <input className={inputStyle} placeholder="Room Number" value={formData.pickupRoomNumber} onChange={(e) => setFormData({...formData, pickupRoomNumber: e.target.value})} />
                              </>
                            )}
                          </div>
                        </div>
                              ...
                              <button
                                ...
                                className={`flex items-center gap-2 rounded-xl px-3 py-1.5 text-xs font-medium transition-all hover:scale-105 active:scale-95 ${
                                  theme === "dark"
                                    ...
                                    : "bg-gray-100 text-gray-700 hover:bg-green-50 hover:text-green-700 border border-transparent"
                                }`}
                              >
                                ...
                                <span className="max-w-[100px] truncate">{addr.type || addr.street}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 rounded-2xl border border-gray-100 p-4 transition-all dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/20 animate-in fade-in zoom-in-95 duration-500">
                          <div className="flex flex-wrap gap-2">
                            {['Home', 'Office', 'Apartment', 'Hotel'].map((type) => (
                              <button
                                key={type}
                                onClick={() => setFormData({ ...formData, dropoffType: type })}
                                className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition-all ${
                          ...
                          <div className="grid grid-cols-2 gap-3 animate-in fade-in duration-300">
                            {formData.dropoffType === 'Home' && (
                              <>
                                <input className={inputStyle} placeholder="Gate Number" value={formData.dropoffGateNumber} onChange={(e) => setFormData({...formData, dropoffGateNumber: e.target.value})} />
                                <input className={inputStyle} placeholder="Gate Color" value={formData.dropoffGateColor} onChange={(e) => setFormData({...formData, dropoffGateColor: e.target.value})} />
                              </>
                            )}
                            {formData.dropoffType === 'Office' && (
                              <>
                                <input className={inputStyle} placeholder="Office Name" value={formData.dropoffOfficeName} onChange={(e) => setFormData({...formData, dropoffOfficeName: e.target.value})} />
                                <input className={inputStyle} placeholder="Floor Number" value={formData.dropoffFloor} onChange={(e) => setFormData({...formData, dropoffFloor: e.target.value})} />
                              </>
                            )}
                            {formData.dropoffType === 'Apartment' && (
                              <>
                                <input className={inputStyle} placeholder="Apartment Name" value={formData.dropoffApartmentName} onChange={(e) => setFormData({...formData, dropoffApartmentName: e.target.value})} />
                                <div className="grid grid-cols-2 gap-2">
                                  <input className={inputStyle} placeholder="Floor" value={formData.dropoffFloor} onChange={(e) => setFormData({...formData, dropoffFloor: e.target.value})} />
                                  <input className={inputStyle} placeholder="Room" value={formData.dropoffRoomNumber} onChange={(e) => setFormData({...formData, dropoffRoomNumber: e.target.value})} />
                                </div>
                              </>
                            )}
                            {formData.dropoffType === 'Hotel' && (
                              <>
                                <input className={inputStyle} placeholder="Hotel Name" value={formData.dropoffHotelName} onChange={(e) => setFormData({...formData, dropoffHotelName: e.target.value})} />
                                <input className={inputStyle} placeholder="Room Number" value={formData.dropoffRoomNumber} onChange={(e) => setFormData({...formData, dropoffRoomNumber: e.target.value})} />
                              </>
                            )}
                          </div>
                        </div>
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className={`rounded-2xl border-2 p-6 text-center transition-all ${
                  ...
                  <p className={`text-xs font-bold uppercase tracking-widest ${theme === "dark" ? "text-green-400" : "text-green-600"}`}>
                    Unique Delivery Code
                  </p>
                  <div className="mt-2 flex items-center justify-center gap-3">
                    <span className={`text-3xl font-black tracking-widest ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
                      {formData.deliveryId}
                    </span>
                    <button 
                      onClick={() => navigator.clipboard.writeText(formData.deliveryId)}
                      className={`rounded-lg p-1.5 transition-all hover:scale-110 active:scale-95 ${
                        theme === "dark" ? "text-gray-400 hover:text-white hover:bg-gray-700/50" : "text-gray-400 hover:text-gray-600 hover:bg-white"
                      }`}
                    >
                ...
                <div className="space-y-4">
                   <label className={`block text-base font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Who is receiving it?</label>
                   <input 
                      className={inputStyle}
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div className="space-y-4">
                   <label className={`block text-base font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Item Details & Photo</label>
                   
                   {/* Photo Capture Section */}
                   <div 
                     onClick={() => setIsCameraOpen(true)}
                     className={`group relative h-48 w-full rounded-2xl border-2 border-dashed overflow-hidden flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ${
                       formData.capturedImage 
                        ? "border-green-500 bg-green-50/10" 
                        : theme === "dark" 
                          ? "border-gray-700 bg-gray-900/40 hover:border-green-500/50 hover:bg-gray-900/60" 
                          : "border-gray-200 bg-gray-50/80 hover:border-green-300 hover:bg-white hover:shadow-lg shadow-gray-200/50"
                     }`}
                   >
                     {formData.capturedImage ? (
                       <>
                         <img src={formData.capturedImage} alt="Captured Item" className="h-full w-full object-cover" />
                         <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="flex flex-col items-center gap-2">
                               ...
                               <span className="text-xs font-bold text-white uppercase tracking-widest">Retake Photo</span>
                            </div>
                         </div>
                       </>
                     ) : (
                       <>
                          <div className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 ${colors.iconBg}`}>
                            <svg className={`h-7 w-7 ${colors.iconColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                              <circle cx="12" cy="13" r="3" strokeWidth={2} />
                            </svg>
                          </div>
                          <span className={`mt-3 text-sm font-bold uppercase tracking-widest ${theme === "dark" ? "text-gray-500" : "text-gray-400"} group-hover:${colors.iconColor}`}>Add a Photo</span>
                       </>
                     )}
                   </div>
                   ...
                   <CameraCapture 
                     ...
                     onCapture={(img) => setFormData(prev => ({ ...prev, capturedImage: img }))}
                   />
              ...
              <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <label className={`block text-base font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Delivery Timing</label>
                    <div className={`flex rounded-2xl p-1.5 ${theme === "dark" ? "bg-gray-900/40" : "bg-gray-100"}`}>
                      {['instant', 'scheduled'].map((type) => (
                        <button
                          key={type}
                          onClick={() => setFormData({ ...formData, deliveryType: type as any })}
                          className={`flex-1 rounded-xl py-3 text-sm font-bold transition-all ${
                            formData.deliveryType === type
                              ? "bg-white text-green-600 shadow-md"
                              : "text-gray-500 hover:text-gray-700"
                          } ${theme === "dark" && formData.deliveryType === type ? "bg-gray-700 text-white" : ""}`}
                        >
                          {type === 'instant' ? 'Deliver Now' : 'Schedule for Later'}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4">
                    <label className={`block text-base font-semibold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>Transportation Method</label>
                        ...
                        { 
                          ...
                          icon: (
                            <svg className="h-9 w-9" viewBox="0 0 512 512" fill="none">
                              <path d="M314.21,482.32,257.44,367.58l-44.89-57.39a72.82,72.82,0,0,1-10.13-37.05V144h15.67a40.22,40.22,0,0,1,40.23,40.22V367.58" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round"/>
                              <path d="M127.9,293.05V218.53S165.16,144,202.42,144" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round"/>
                              <line x1="370.1" y1="274.42" x2="304" y2="231" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round"/>
                              <line x1="170.53" y1="478.36" x2="224" y2="400" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round"/>
                              <circle cx="258.32" cy="69.48" r="37.26" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                          )
                        },
                        { 
                        ...
                        { 
                          ...
                          icon: (
                            <svg className="h-10 w-10" viewBox="0 0 24 24" fill="none">
                              <path fillRule="evenodd" clipRule="evenodd" d="M5.27097 8.214C6.02074 6.312 6.92262 5 8.96427 5H14.4369C16.4766 5 17.3795 6.312 18.1302 8.214L18.88 10.249C19.4387 10.2559 19.9519 10.5661 20.2284 11.064C20.3911 11.3476 20.4763 11.6709 20.4751 12V14.624C20.4824 15.2367 20.1808 15.8098 19.6776 16.14C19.4382 16.2929 19.162 16.3739 18.88 16.374H4.52022C4.23827 16.3739 3.96199 16.2929 3.72267 16.14C3.21939 15.8098 2.91786 15.2367 2.92512 14.624V12C2.92406 11.6713 3.00927 11.3483 3.17179 11.065C3.44833 10.5671 3.96155 10.2569 4.52022 10.25L5.27097 8.214Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                              <path d="M6.86629 16.375C6.86629 15.9608 6.53051 15.625 6.11629 15.625C5.70208 15.625 5.36629 15.9608 5.36629 16.375H6.86629ZM6.11629 17.688H5.36629V17.687H6.11629ZM5.85012 12.75H6.82512V14.25H5.85012V12.75ZM16.5751 12.75H17.5501V14.25H16.5751V12.75ZM20.4285 16.14V17.687H18.9285V16.14H20.4285Z" fill="currentColor"/>
                            </svg>
                          )
                        }
                      ].filter(method => {
                        ...
                        <button
                          ...
                          className={`group flex flex-col items-center justify-center rounded-2xl border-2 p-5 transition-all duration-300 ${
                            formData.transportMethod === method.id
                              ? "border-green-500 bg-green-50/50 scale-[1.02] shadow-sm shadow-green-900/10"
                              : theme === "dark" 
                          ...
                          <div 
                            className={`transition-all duration-300 ${
                              formData.transportMethod === method.id 
                                ...
                                : theme === "dark" ? "text-gray-500 group-hover:text-gray-300" : "text-gray-400 group-hover:text-green-500"
                            }`} 
                          >
                            {method.icon}
                          </div>
                  ...
                  {/* Pricing Summary */}
                  <div className={`mt-6 rounded-2xl p-6 transition-all ${
                    theme === "dark" ? "bg-gray-900/60" : "bg-green-600 shadow-xl shadow-green-500/20"
                  }`}>
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        <p className={`text-[10px] font-black uppercase tracking-widest ${theme === "dark" ? "text-green-400" : "text-green-200 opacity-80"}`}>
                          Estimated Distance
                        </p>
                      ...
                      <div className="text-right space-y-1">
                        ...
                        <p className="text-2xl font-black text-white italic">
                          {isCalculating ? (
                            ...
                            `${formData.calculatedFee.toLocaleString()} ${systemConfig?.currency || 'RWF'}`
                          )}
                        </p>
                      </div>
                    </div>
        ...
        <div
          className={`flex w-full flex-col-reverse gap-3 px-6 py-5 sm:flex-row sm:justify-end sm:px-8 ${
            theme === "dark" ? "border-t border-gray-700/50" : "border-t border-gray-100"
          }`}
        >
          {step === 1 ? (
             <button
