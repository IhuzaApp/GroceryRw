...
const AddPaymentMethodModal: React.FC<AddPaymentMethodModalProps> = ({
  ...
  const [method, setMethod] = useState<
    "Visa" | "Mastercard" | "MTN MoMo" | "Airtel"
  >("Visa");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        t("payment.addSuccess") || "Payment method added successfully!"
      );
      onSuccess();
      onClose();
      // Reset form
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-end justify-center bg-black/60 backdrop-blur-sm transition-opacity sm:items-center sm:p-4">
      <div
        className={`w-full max-w-lg transform rounded-t-[2.5rem] p-8 shadow-2xl transition-all sm:rounded-[2rem] ${
          theme === "dark" ? "bg-gray-900 text-white" : "bg-white text-gray-900"
        }`}
      >
        <div className="mb-6 flex items-center justify-between">
          <h3 className="text-xl font-black tracking-tight">
            {t("payment.addMethod") || "Add Payment Method"}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-gray-400">
                {t("payment.cardHolder") || "Names on Account"}
              </label>
              <input
                required
                type="text"
                ...
                className={`w-full rounded-2xl border-2 px-5 py-2.5 text-xs font-medium transition-all focus:outline-none focus:ring-4 focus:ring-green-500/10 ${
                  theme === "dark"
                    ? "border-gray-800 bg-gray-800 focus:border-green-500"
                    : "border-gray-100 bg-gray-50 focus:border-green-500"
                }`}
              />
            </div>
          ...
          <div className="flex items-center gap-3 py-2">
            <button
              ...
              className={`flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 focus:outline-none ${
                isDefault ? "bg-green-500" : "bg-gray-200 dark:bg-gray-700"
              }`}
            >
              <span
