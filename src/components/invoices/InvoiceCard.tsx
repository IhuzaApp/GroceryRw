...
const InvoiceCard: React.FC<InvoiceCardProps> = ({
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium border ${config.color}`}>
  ...
  return (
    <div className={`mb-6 rounded-2xl border ${theme === "dark" ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-white shadow-sm"} transition-all duration-200 hover:shadow-md`}>
      {/* Header */}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>
              {formatDate(invoice.created_at)} • {invoice.order_type === "reel" ? "Reel Order" : "Regular Order"}
            </p>
          </div>
              ...
              <div className="mt-3">
                {hasProof(invoice) ? (
                  <div className="flex items-center">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/30 mr-3">
