      ...
      <section className="relative -mx-4 -mt-6 overflow-hidden shadow-2xl sm:mx-0 sm:mt-0 sm:rounded-[3rem]">
        <div className="relative h-[20rem] w-full sm:h-[30rem]">
          ...
          <div className="absolute left-6 right-6 top-8 flex items-center justify-between">
            <Link
              href="/Recipes"
              className="group flex h-11 w-11 items-center justify-center rounded-2xl border border-white/20 bg-white/10 text-white backdrop-blur-xl transition-all hover:border-emerald-500 hover:bg-emerald-500 active:scale-90"
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 sm:p-10">
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              <span className="rounded-full border border-white/20 bg-white/20 px-4 py-1 text-[10px] font-black uppercase tracking-widest text-white backdrop-blur-md">
                {meal.strArea} Origin
              </span>
            </div>
            ...
            {meal.strTags && (
              <div className="mt-6 flex flex-wrap gap-2">
                {meal.strTags.split(",").map((tag) => (
                  <span
                    key={tag}
                    className="rounded-xl border border-white/10 bg-white/10 px-3 py-1.5 text-xs font-bold text-white backdrop-blur-md"
                  >
                    #{tag.trim()}
                  </span>
                ))}
      ...
      <div className="mt-8 space-y-8 px-0">
        ...
        <div className="grid gap-8 lg:grid-cols-5">
          ...
          <section className="lg:col-span-2">
            <div
              className={`sticky top-8 rounded-[2rem] border-2 p-6 sm:rounded-[2.5rem] sm:p-8 ${
                isDark
            ...
            >
              ...
              >
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span
                    className={`rounded-full bg-emerald-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-emerald-500`}
                  >
                    {ingredients.length} items
                  </span>
                </div>
                <div
                  className={`flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 transition-transform duration-300 lg:hidden ${
              ...
              <div
                className={`${ingredientsOpen ? "block" : "hidden lg:block"}`}
              >
          ...
          <section className="lg:col-span-3">
            <div
              className={`rounded-[2rem] border-2 p-6 sm:rounded-[2.5rem] sm:p-8 ${
                isDark
                  ? "border-white/5 bg-[#171717]"
                  : "border-black/5 bg-white"
              }`}
            >
              ...
              <div
                className={`${instructionsOpen ? "block" : "hidden lg:block"}`}
              >
              ...
              {youtubeId && (
                <div className="mt-12 overflow-hidden rounded-[2rem] border-2 border-white/5 bg-black/20">
                  <div className="p-6">
                    <h3 className="mb-2 text-lg font-black text-white">
                      Watch Tutorial
                    </h3>
                    <p className="mb-6 text-sm text-white/60">
                      Learn the exact techniques used in this recipe.
                    </p>
