...
export default function PetBusinessDashboard() {
  ...
  if (vendorData?.disabled) {
    return (
      <div
        className={`min-h-screen ${theme === "dark" ? "bg-[#0A0A0A]" : "bg-white"
          }`}
      >
  ...
  const handleProcessRequest = async (adoption: any, action: 'ACCEPT' | 'REJECT') => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`Adoption ${action === 'ACCEPT' ? 'accepted' : 'Rejected'}.`, { id: toastId });
        if (vendorData?.id) {
          fetchAdoptions(vendorData.id);
        }
      } else {
  ...
  return (
    <div
      className={`min-h-screen pb-24 md:ml-20 ${theme === "dark" ? "bg-[#0A0A0A] text-white" : "bg-[#FAFAFA] text-black"
        }`}
    >
      <PetDashboardHeader
      ...
      <div className="mx-auto max-w-[1600px] px-6 py-10">
        ...
        <div className="mb-8 flex gap-8 border-b border-gray-200/10 dark:border-white/5">
          <button
            ...
            className={`pb-4 text-sm font-black uppercase tracking-widest transition-all ${activeTab === "pets"
          ...
          <button
            ...
            className={`pb-4 text-sm font-black uppercase tracking-widest transition-all ${activeTab === "interests"
              ? "border-b-2 border-green-500 text-green-500"
              : "text-gray-500 hover:text-gray-400"
              }`}
          >
        ...
        {activeTab === "pets" ? (
          <div className="space-y-6">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              <div
                className={`flex max-w-md flex-1 items-center rounded-2xl border px-4 py-3 shadow-sm ${theme === "dark"
              ...
              >
                ...
                <input
                  ...
                  className={`ml-3 flex-1 bg-transparent text-sm font-bold outline-none placeholder:text-gray-400 ${theme === "dark" ? "text-white" : "text-gray-900"
                    }`}
                />
              </div>
          ...
          <div className="space-y-4">
            {adoptions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32 text-center">
                <div
                  className={`mb-6 flex h-20 w-20 items-center justify-center rounded-[2rem] ${theme === "dark" ? "bg-white/5" : "bg-white shadow-xl"
                    }`}
                >
...
function StatsCard({ label, value, icon, color, theme }: any) {
  ...
  return (
    <div
      className={`rounded-[2.5rem] border p-6 transition-all hover:translate-y-[-4px] hover:shadow-2xl ${theme === "dark"
    ...
    >
      ...
      <h3
        className={`font-outfit text-2xl font-black ${theme === "dark"
...
function PetManagementItem({
  ...
  return (
    <div
      className={`flex items-center justify-between rounded-[2.5rem] border p-4 transition-all hover:shadow-xl ${theme === "dark"
    ...
    >
      <div className="flex items-center gap-5">
        ...
        <div>
          <h4
            className={`font-outfit text-xl font-black ${theme === "dark"
              ? "!text-white text-white"
              : "!text-gray-900 text-gray-900"
              }`}
          >
          ...
          <div className="mt-2 flex items-center gap-3">
            <div
              className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${pet.status === "available"
      ...
      <div className="flex flex-1 items-center gap-5">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-widest ${adoption.status === "PAID" ? "bg-blue-500/10 text-blue-500" :
              adoption.status === "ACCEPTED" ? "bg-green-500/10 text-green-500" :
                adoption.status === "DELIVERED" ? "bg-gray-500/10 text-gray-400" :
                  "bg-red-500/10 text-red-500"
              }`}>
              {adoption.status === "ACCEPTED" ? "Ready for Delivery" : adoption.status}
            </span>
          </div>
          <p className="text-xs font-black uppercase tracking-widest !text-gray-500 text-gray-500">
