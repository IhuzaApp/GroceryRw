...
const InvoiceProofModal: React.FC<InvoiceProofModalProps> = ({
  ...
  const renderContent = () => {
    return (
      <div className="fixed inset-0 z-[9999] flex items-end justify-center p-0 sm:items-center sm:p-4" onClick={handleClose}>
        {/* Backdrop */}
        ...
        <div className={`relative z-10 flex w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl border-0 shadow-2xl sm:max-h-[90vh] sm:rounded-2xl sm:border ${theme === "dark" ? "bg-gray-800 sm:border-gray-700" : "bg-white sm:border-gray-200"}`} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={`flex flex-shrink-0 items-center justify-between px-6 py-6 sm:px-8 ${theme === "dark" ? "border-b border-gray-700" : "border-b border-gray-200"}`}>
            <div className="flex items-center gap-3">
              <div className={`rounded-full p-2 ${theme === "dark" ? "bg-blue-600" : "bg-blue-100"}`}>
                <svg className={`h-6 w-6 ${theme === "dark" ? "text-white" : "text-blue-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <div>
                <h3 className={`text-lg font-bold md:text-xl ${theme === "dark" ? "text-gray-100" : "text-gray-800"}`}>
                  {uploading ? "Uploading Invoice Proof..." : "Invoice Proof Required"}
                </h3>
          ...
          <div className={`max-h-[70vh] flex-1 overflow-y-auto px-6 py-8 sm:px-8 ${theme === "dark" ? "bg-gray-800 text-gray-100" : "bg-white"}`}>
            <div className="space-y-4">
              {/* Instructions */}
              <div className={`rounded-xl border-l-4 p-4 ${theme === "dark" ? "border-blue-500 bg-blue-900/20 text-blue-300" : "border-blue-500 bg-blue-50 text-blue-800"}`}>
                <div className="flex items-start gap-3">
                  <svg className={`mt-0.5 h-5 w-5 flex-shrink-0 ${theme === "dark" ? "text-blue-400" : "text-blue-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <div className="flex-1">
                    <h4 className="font-semibold">Payment Complete - Invoice Proof Required</h4>
                    <p className="mt-1 text-sm">
                      Please take a clear photo of the payment receipt or invoice before proceeding to delivery. This serves as proof of purchase.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {!capturedImage && !uploading && (
                <div className={`rounded-xl border-2 p-6 text-center ${theme === "dark" ? "border-gray-700 bg-gray-900/50" : "border-gray-200 bg-gray-50"}`}>
                  ...
                  <h3 className={`mb-2 text-lg font-semibold ${theme === "dark" ? "text-gray-100" : "text-gray-800"}`}>
              ...
              {capturedImage && !uploading && (
                <div className={`rounded-xl border-2 p-4 ${theme === "dark" ? "border-green-600 bg-green-900/20" : "border-green-200 bg-green-50"}`}>
                  <div className="mb-3 flex items-center gap-2">
                    <svg className={`h-5 w-5 ${theme === "dark" ? "text-green-400" : "text-green-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="relative mx-auto h-64 w-full overflow-hidden rounded-lg sm:h-96">
                    <Image src={capturedImage} alt="Invoice proof" fill className="object-contain" />
                  </div>
          ...
          {/* Footer */}
          {capturedImage && !uploading && (
            <div className={`flex w-full flex-col-reverse gap-3 px-6 py-5 sm:flex-row sm:justify-end sm:px-8 ${theme === "dark" ? "border-t border-gray-700" : "border-t border-gray-200"}`}>
              <button
