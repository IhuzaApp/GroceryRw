...
const TierBadge: React.FC<{ level: string; type: string; isAchieved: boolean }> = ({ level, type, isAchieved }) => {
  ...
  return (
    <div className={`relative flex h-14 w-14 items-center justify-center transition-all duration-300 ${!isAchieved ? "grayscale" : ""}`}>
      {/* Dynamic Glow Aura */}
      ...
      <div className={`relative flex h-12 w-12 items-center justify-center rounded-full border-2 shadow-lg backdrop-blur-md transition-all duration-300 
        ...
        ${isAchieved ? `shadow-xl ${config.glow}` : "opacity-40"}`}
      >
  ...
  const renderAchievementCard = (a: Achievement, isAchieved: boolean) => (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        {isAchieved ? (
          <div className="flex h-8 items-center px-4 rounded-full bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-widest">
            Achieved
          </div>
        ) : (
      ...
      <div className={`absolute -right-4 -bottom-4 h-24 w-24 rounded-full blur-[40px] opacity-10 transition-all duration-700 group-hover:scale-150 group-hover:opacity-20
        ${a.level.toLowerCase() === 'gold' ? 'bg-yellow-500' : a.level.toLowerCase() === 'platinum' ? 'bg-cyan-500' : 'bg-orange-500'}`} 
      />
    </div>
  );
