  ...
  if (!goals) {
    return (
      <Panel shaded bordered bodyFill className="p-3 sm:p-4">
        <h3 className="mb-3 sm:mb-4 text-base sm:text-lg font-semibold">Earnings Goals</h3>
        <div className="flex flex-col items-center justify-center py-6 sm:py-8 text-center">
          <svg
            ...
            stroke="currentColor"
            strokeWidth="2"
            className="mb-3 sm:mb-4 h-8 w-8 sm:h-12 sm:w-12 text-gray-400"
          >
          ...
          <h4 className="mb-2 text-base sm:text-lg font-medium text-gray-900">
      ...
      <Panel shaded bordered bodyFill className="p-3 sm:p-4">
        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            <div key={index}>
              <div className="mb-1 flex items-center justify-between">
                <div className="flex items-center min-w-0 flex-1">
                  <span className="text-xs sm:text-sm font-medium truncate">{item.goal}</span>
                  <Button
                    appearance="link"
                    size="xs"
                    onClick={() => handleEditGoal(item)}
                    className="ml-2 p-0 text-blue-500 flex-shrink-0"
                  >
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
          ...
          <div className="mt-4 sm:mt-6 border-t pt-3 sm:pt-4">
            <h3 className="mb-2 sm:mb-3 text-sm sm:text-base font-medium">Tips to Increase Earnings</h3>
            {loadingTips ? (
              <div className="space-y-2">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="flex items-start gap-2">
                    <div className="mt-0.5 flex h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-400">
                      {i}
                    </div>
                    <div className="h-3 sm:h-4 w-32 sm:w-48 animate-pulse rounded bg-gray-200"></div>
                  </div>
                ))}
              </div>
            ) : earningsTips ? (
              <ul className="space-y-2 text-xs sm:text-sm">
                {earningsTips.tips.map((tip, index) => (
                  <li key={index} className="flex items-start gap-2">
                    <div className="mt-0.5 flex h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0 items-center justify-center rounded-full bg-green-100 text-green-600">
                      {index + 1}
                    </div>
                    <span>{tip}</span>
                  </li>
