    ...
    <div
      className={`group relative overflow-hidden rounded-[2rem] p-6 transition-all duration-500 hover:shadow-2xl ${
        isDark 
          ...
          : "bg-white border border-black/5 shadow-xl hover:shadow-indigo-500/10"
      }`}
    >
      {/* Background Decorative Glow */}
      <div className="absolute -right-12 -top-12 h-32 w-32 rounded-full bg-indigo-500/10 blur-3xl transition-opacity group-hover:opacity-100 opacity-50" />
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="mt-4 flex items-center gap-2">
                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-tight ${
                  isDark ? "bg-amber-500/20 text-amber-500" : "bg-amber-100 text-amber-700"
                }`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className={`rounded-xl p-3 ${isDark ? "bg-white/5" : "bg-black/5"}`}>
                ...
                <p className="text-xl font-black text-emerald-500">{completedTransactions}</p>
              </div>
              <div className={`rounded-xl p-3 ${isDark ? "bg-white/5" : "bg-black/5"}`}>
                <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Pending</p>
                <p className="text-xl font-black text-amber-500">{pendingTransactions}</p>
              </div>
            </div>
          </div>
        )}
