...
const TierBadge: React.FC<{
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`relative flex h-8 w-8 items-center justify-center rounded-full border border-white/20 shadow-lg backdrop-blur-md 
  ...
  const renderAchievementCard = (a: Achievement, isAchieved: boolean) => (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        {isAchieved ? (
          <div className="flex h-6 items-center rounded-full bg-emerald-500/10 px-3 text-[8px] font-black uppercase tracking-widest text-emerald-500">
            Achieved
          </div>
        ) : (
    ...
    <div className="space-y-8 pb-6">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          >
            <span className="mb-1 block text-sm">{stat.icon}</span>
            <div className={`text-base font-black ${stat.color}`}>
              {stat.val}
            </div>
            <div className="mt-0.5 text-[8px] font-black uppercase tracking-widest opacity-40">
              {stat.label}
            </div>
          </div>
        ))}
