...
const AchievementBadgesMobile: React.FC = () => {
  ...
  const getBadgeIcon = (level: string) => {
    switch (level) {
      case 'bronze':
        return (
          <svg viewBox="-3.5 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" width="24" height="24">
    ...
    return (
      <div className="w-full">
        ...
        <div className="flex items-center space-x-3 mb-4">
          <div className={`w-8 h-8 rounded-lg ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} animate-pulse`}></div>
          <div>
            <div className={`h-4 w-24 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded animate-pulse mb-1`}></div>
            <div className={`h-3 w-20 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded animate-pulse`}></div>
          </div>
        </div>
        
        {/* Mobile Summary Stats Skeleton */}
        <div className={`mb-4 grid grid-cols-3 gap-2 rounded-xl p-3 ${theme === "dark" ? "bg-gray-800" : "bg-gray-100"} animate-pulse`}>
          {[1, 2, 3].map((i) => (
            <div key={i} className="text-center">
              <div className={`w-8 h-8 rounded-full mx-auto mb-2 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"}`}></div>
              <div className={`h-4 w-6 mx-auto mb-1 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded`}></div>
              <div className={`h-2 w-8 mx-auto ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded`}></div>
            </div>
          ))}
        </div>
        
        {/* Mobile Achievement Cards Skeleton */}
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className={`rounded-xl p-3 ${theme === "dark" ? "bg-gray-800" : "bg-gray-100"} animate-pulse`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2 flex-1">
                  <div className={`w-8 h-8 rounded-lg ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"}`}></div>
                  <div className="flex-1">
                    <div className={`h-3 w-20 mb-1 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded`}></div>
                    <div className={`h-2 w-16 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded`}></div>
                  </div>
                </div>
                <div className={`w-8 h-4 ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded-full`}></div>
              </div>
              <div className={`h-1.5 w-full ${theme === "dark" ? "bg-gray-700" : "bg-gray-200"} rounded-full`}></div>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="w-full">
        <div className="flex flex-col items-center justify-center py-8 text-center px-4">
          ...
          <p className={`text-sm ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>Unable to load achievements</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center space-x-2 mb-3">
        <div className={`p-1.5 rounded-lg ${theme === "dark" ? "bg-yellow-500/20" : "bg-yellow-100"}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-4 w-4 text-yellow-500">
      ...
      <div className={`mb-4 grid grid-cols-3 gap-2 rounded-xl p-3 ${theme === "dark" ? "bg-gradient-to-r from-gray-800 to-gray-700 border border-gray-600" : "bg-gradient-to-r from-white to-gray-50 border border-gray-200"} shadow-sm`}>
        <div className="text-center">
          <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full mb-1 ${theme === "dark" ? "bg-green-500/20" : "bg-green-100"}`}>
            <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4 text-green-500">
              <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div className="text-sm font-bold text-green-500">{summary.totalAchieved}</div>
          <div className={`text-xs font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>Earned</div>
        </div>
        <div className="text-center">
          <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full mb-1 ${theme === "dark" ? "bg-orange-500/20" : "bg-orange-100"}`}>
            <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4 text-orange-500">
              <path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div className="text-sm font-bold text-orange-500">{summary.totalPending}</div>
          <div className={`text-xs font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>In Progress</div>
        </div>
        <div className="text-center">
          <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full mb-1 ${theme === "dark" ? "bg-purple-500/20" : "bg-purple-100"}`}>
          ...
          <div className="text-sm font-bold text-purple-500">{achieved.length + pending.length}</div>
          <div className={`text-xs font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>Total</div>
        </div>
      </div>

      {/* Mobile Achieved Badges */}
      {achieved.length > 0 && (
        <div className="mb-4">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className={`text-sm font-bold ${theme === "dark" ? "text-green-400" : "text-green-600"}`}>
              Achieved This Month
            </h4>
          </div>
          <div className="space-y-2">
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      <div className="flex items-center justify-center w-8 h-8 flex-shrink-0 rounded-lg bg-white/10 backdrop-blur-sm">
      ...
      {pending.length > 0 && (
        <div>
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className={`text-sm font-bold ${theme === "dark" ? "text-blue-400" : "text-blue-600"}`}>
              Working Towards
            </h4>
          </div>
          <div className="space-y-2">
            {pending.slice(0, 4).map((achievement, index) => (
              <div
                key={index}
                className={`relative overflow-hidden rounded-xl p-3 ${
                  theme === "dark" 
              ...
              >
                <div className="relative">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      <div className="flex items-center justify-center w-8 h-8 flex-shrink-0 rounded-lg bg-white/5 backdrop-blur-sm">
                        {getBadgeIcon(achievement.level)}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1 mb-1">
          ...
          {pending.length > 4 && (
            <div className="mt-3 text-center">
              <div className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium ${
                ...
                <span>And {pending.length - 4} more badges to unlock...</span>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Mobile No achievements message */}
      {achieved.length === 0 && pending.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 text-center px-4">
          <div className={`relative mb-4 ${theme === "dark" ? "text-gray-600" : "text-gray-300"}`}>
            <div className="text-4xl mb-2">🏆</div>
            <div className="absolute -top-1 -right-1 w-3 h-3 bg-yellow-400 rounded-full animate-pulse"></div>
          </div>
          <h4 className={`mb-2 text-lg font-bold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
            Begin Your Journey
          </h4>
          <p className={`text-xs max-w-xs ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>
            Complete orders, earn great ratings, and reach monthly targets to unlock your first achievement badges!
          </p>
          <div className={`mt-4 px-4 py-2 rounded-full ${theme === "dark" ? "bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/30" : "bg-gradient-to-r from-yellow-100 to-orange-100 border border-yellow-200"}`}>
            <span className={`text-xs font-medium ${theme === "dark" ? "text-yellow-300" : "text-yellow-700"}`}>
