  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Syncing Logistics...</p>
      </div>
    );
  }
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-2 gap-6">
          {deliveryMetrics.map((metric, index) => {
            ...
            const accentStyles = {
              blue: isDark ? "bg-blue-500/10 text-blue-400 ring-blue-500/20 glow-blue" : "bg-blue-50 text-blue-600 ring-blue-100",
              emerald: isDark ? "bg-emerald-500/10 text-emerald-400 ring-emerald-500/20 glow-emerald" : "bg-emerald-50 text-emerald-600 ring-emerald-100",
              amber: isDark ? "bg-amber-500/10 text-amber-400 ring-amber-500/20 glow-amber" : "bg-amber-50 text-amber-600 ring-amber-100",
              purple: isDark ? "bg-purple-500/10 text-purple-400 ring-purple-500/20 glow-purple" : "bg-purple-50 text-purple-600 ring-purple-100",
            ...
            return (
              ...
              >
                <div className="mb-6">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-[1.25rem] ring-1 transition-transform group-hover/item:-rotate-6 ${accentStyles}`}>
                ...
                <div>
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className={`text-[10px] font-black uppercase tracking-widest ${isDark ? "text-white/20" : "text-gray-400"}`}>
                      {metric.unit}
                    </span>
                  </div>
                  <p className={`mt-2 text-[9px] font-black uppercase tracking-[0.2em] ${isDark ? "text-white/40" : "text-gray-500"}`}>
                    {metric.label}
                  </p>
                </div>

                {/* Subtle Decorative Flow */}
                <div className={`absolute -right-4 -bottom-4 h-16 w-16 rounded-full blur-2xl opacity-0 transition-opacity duration-500 group-hover/item:opacity-20 ${
                  metric.accent === "blue" ? "bg-blue-500" : metric.accent === "emerald" ? "bg-emerald-500" : metric.accent === "amber" ? "bg-amber-500" : "bg-purple-500"
                }`} />
              </div>
            );
          })}
