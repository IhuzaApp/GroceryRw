  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Calculating Milestones...</p>
      </div>
    );
  }
              ...
              <div key={index} className="group/item space-y-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-[1.25rem] ring-1 transition-transform group-hover/item:rotate-6 ${iconBgMap[goal.accent]}`}>
                      <Icon className={`h-6 w-6 ${textColorMap[goal.accent]}`} />
                    </div>
                    <div>
                      <span className={`text-sm font-black tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}>
                        {goal.label}
                      </span>
                      <p className={`text-[10px] font-black uppercase tracking-widest ${isDark ? "text-white/20" : "text-gray-400"}`}>
                        {goal.description}
                      </p>
                    </div>
                  </div>
                  {isComplete ? (
                    ...
                    <div className={`text-right ${isDark ? "text-white/40" : "text-gray-500"}`}>
                ...
                <div className="space-y-3">
                  ...
                  <div className={`h-3 w-full overflow-hidden rounded-full p-0.5 ${isDark ? "bg-white/5 shadow-inner" : "bg-gray-100 shadow-inner"}`}>
                    <div
                      className={`h-full rounded-full bg-gradient-to-r transition-all duration-1000 ease-out ${colorMap[goal.accent]}`}
                  ...
                  <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-[0.25em]">
                    ...
                    {!isComplete && (
                      <span className="text-emerald-500">
                        {formatCurrencySync(goal.data.target - goal.data.current)} To Target
                      </span>
                    )}
                  </div>
                </div>
