      ...
      >
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {/* Icon */}
              <span className={`transition-all duration-300 ${isActive ? "scale-110" : "scale-100 group-hover:scale-110"}`}>
                {tab.icon}
              </span>
              ...
              {isActive && (
                <span className="absolute inset-0 rounded-[1.5rem] bg-white opacity-10 animate-pulse" />
              )}
            </button>
          );
        })}
