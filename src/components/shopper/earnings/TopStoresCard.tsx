        ...
        {isLoading ? (
          ...
          <div className="space-y-8">
            {stores.map((item, index) => {
              const storeName =
                item.store || item.name || `Channel ${index + 1}`;
                ...
                <div key={index} className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex h-12 w-12 items-center justify-center rounded-[1.25rem] ring-1 transition-transform group-hover:scale-105 ${iconBgMap[accent]}`}
                      >
                        <ShoppingCart
                          className={`h-6 w-6 ${textColorMap[accent]}`}
                        />
                      </div>
                    ...
                    <div className="text-right">
                      <span
                        className={`text-sm font-black tabular-nums ${textColorMap[accent]}`}
                      >
