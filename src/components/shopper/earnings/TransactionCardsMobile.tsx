...
const TransactionCardsMobile: React.FC<TransactionCardsMobileProps> = ({
  ...
  const filteredTransactions = transactions
    .filter((t) => activeFilter === "all" || getTransactionCategory(t.type) === activeFilter)
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Syncing Wallet...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {!filteredTransactions.length ? (
        <div className={`rounded-[2.5rem] border-2 border-dashed p-12 text-center ${isDark ? "border-white/5" : "border-black/5"}`}>
          <p className="text-sm font-bold opacity-30 uppercase tracking-widest">No matching logs</p>
        </div>
      ) : (
        <div className="space-y-4">
          {filteredTransactions.map((t) => {
            ...
            const accentColor = isIncome ? "emerald" : category === "sent" ? "rose" : "indigo";
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-xl text-[10px] font-black ${
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest opacity-40">{formatDate(t.date)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={`text-base font-black ${isIncome ? "text-emerald-500" : "text-rose-500"}`}>
                      {isIncome ? "+" : "-"}{formatCurrencySync(t.amount)}
                    </p>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-4 border-t border-black/5 dark:border-white/5">
                  <span className={`inline-flex rounded-lg px-2 py-0.5 text-[8px] font-black uppercase tracking-widest ${
                    isDark ? "bg-white/5 text-white/40" : "bg-black/5 text-black/40"
                  }`}>
                    {t.type}
                  </span>
                ...
                {/* Glow bar */}
                <div className={`absolute left-0 top-0 h-full w-1 ${isIncome ? "bg-emerald-500" : "bg-rose-500"} opacity-40`} />
              </div>
            );
          })}
        </div>
