...
const PickupConfirmationScanner: React.FC<PickupConfirmationScannerProps> = ({
  ...
  const handleConfirm = async () => {
    const isManualMatch = showManualInput && normalizeOrderId(manualId) === expectedNormalized;
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 pt-8 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black uppercase tracking-tight text-[var(--text-primary)]">Verify Pickup</h3>
        ...
        <div className="px-8 pb-8">
          ...
          {!showManualInput ? (
            <div className="space-y-6">
              ...
              <div className="relative aspect-square overflow-hidden rounded-[2.5rem] bg-black shadow-2xl ring-1 ring-white/10">
                ...
                <div className="absolute inset-0 flex items-center justify-center p-12">
                  <div className="relative h-full w-full">
                    ...
                    {/* Corners */}
                    <div className="cyber-corner left-0 top-0 border-l-4 border-t-4 rounded-tl-xl" />
                    <div className="cyber-corner right-0 top-0 border-r-4 border-t-4 rounded-tr-xl" />
                    <div className="cyber-corner left-0 bottom-0 border-l-4 border-b-4 rounded-bl-xl" />
                    <div className="cyber-corner right-0 bottom-0 border-r-4 border-b-4 rounded-br-xl" />
                    
                    {/* Center Target */}
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/20 backdrop-brightness-125">
                      <span className={`text-2xl font-black tracking-tighter transition-all duration-300 ${
                        matchResult === "match" ? "scale-125 text-emerald-500" : "text-white/40"
                      }`}>
                        #{expectedNormalized}
                      </span>
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
                  <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/60 px-4 py-2 backdrop-blur-md">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white">
                      {matchResult === "match" ? "Order Verified" : "Ready to Scan"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center">
                {matchResult === "no_match" ? (
                  ...
                  <p className={`text-xs font-bold opacity-40 ${theme === "dark" ? "text-white" : "text-black"}`}>
            ...
            <div className="space-y-6 pt-4">
              <div className="text-center">
                <h4 className="text-lg font-black uppercase tracking-tight text-[var(--text-primary)]">Manual Entry</h4>
                <p className={`text-xs opacity-60 ${theme === "dark" ? "text-white" : "text-black"}`}>
                  Enter order number exactly as seen on receipt
                </p>
              </div>
              
              <div className={`relative overflow-hidden rounded-[2rem] p-8 border ${
                theme === "dark" ? "bg-white/5 border-white/10" : "bg-black/5 border-black/5"
              }`}>
                <div className="flex flex-col items-center">
                  <span className="mb-2 text-[9px] font-black uppercase tracking-[0.3em] opacity-40">Required ID</span>
                  <span className="text-3xl font-black tracking-tighter text-emerald-500">#{expectedNormalized}</span>
                </div>
                ...
                <div className="mt-8">
                  ...
                  />
                  <div className={`mx-auto mt-2 h-1 w-24 rounded-full ${
                    manualId ? "bg-emerald-500 shadow-[0_0_10px_#10b981]" : "bg-white/10"
                  }`} />
                </div>
              </div>

              {manualId && normalizeOrderId(manualId) !== expectedNormalized && (
          ...
          <div className="mt-10 flex flex-col gap-3">
            {matchResult === "match" || (showManualInput && normalizeOrderId(manualId) === expectedNormalized) ? (
              <button
                onClick={handleConfirm}
                disabled={confirming}
                className="w-full flex items-center justify-center gap-3 rounded-[1.5rem] bg-emerald-600 py-5 text-[11px] font-black uppercase tracking-[0.2em] text-white shadow-[0_15px_30px_rgba(16,185,129,0.3)] transition-all hover:bg-emerald-700 active:scale-95 disabled:opacity-50"
              >
              ...
              !showManualInput && showManualButton && (
                <button
                  ...
                  className={`w-full rounded-[1.5rem] border py-5 text-[10px] font-black uppercase tracking-[0.2em] transition-all hover:scale-[1.02] active:scale-95 ${
                    theme === "dark" ? "border-white/10 text-white/40 hover:text-white" : "border-black/5 text-black/40 hover:text-black"
                  }`}
                >
                  Enter Manually
                </button>
