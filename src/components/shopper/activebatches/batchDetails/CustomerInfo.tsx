...
export default function CustomerInfo({
  ...
  const headerClasses = `flex items-center justify-between px-5 py-4 border-b ${
    theme === "dark" ? "border-white/5 bg-white/[0.02]" : "border-black/5 bg-gray-50/50"
  }`;
    ...
    <div className={containerClasses}>
      <div className={headerClasses}>
        <div className="flex items-center gap-3">
          ...
          <h2 className={`text-sm font-black uppercase tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Customer</h2>
        </div>
      </div>
          ...
          <div key={customer.id || index} className="p-5 space-y-5">
            <div className="flex items-start gap-4">
              <div className={`relative h-14 w-14 flex-shrink-0 rounded-full border-2 p-0.5 ${
                theme === "dark" ? "border-white/10 bg-white/5" : "border-black/5 bg-gray-50"
              }`}>
                <Image
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className={`truncate text-base font-black tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                ...
                <div className="mt-3 flex items-start gap-2 rounded-xl bg-gray-50/50 p-3 dark:bg-white/[0.02]">
                  ...
                  <div className="flex-1">
                    ...
                    <p className={`mt-0.5 text-xs font-bold leading-relaxed ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => handleDirectionsClick(`${customer.address?.street || ""}, ${customer.address?.city || ""}`)}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-600 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-emerald-500/30 transition-all hover:scale-[1.01] active:scale-[0.99]"
              >
