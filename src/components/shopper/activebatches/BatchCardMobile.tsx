...
interface Order {
  ...
  reel?: { id: string; title: string; restaurant_id?: string | null; user_id?: string | null };
  quantity?: number;
...
const ORDER_TYPE_CFG = {
  regular:    { label: "Regular",    gradient: "from-emerald-500 to-green-600",  dot: "#34d399" },
  reel:       { label: "Reel",       gradient: "from-violet-500 to-purple-600",  dot: "#a78bfa" },
  restaurant: { label: "Restaurant", gradient: "from-orange-500 to-red-500",     dot: "#fb923c" },
  combined:   { label: "Combined",   gradient: "from-amber-500 to-yellow-500",   dot: "#fbbf24" },
  business:   { label: "Business",   gradient: "from-blue-500 to-indigo-600",    dot: "#60a5fa" },
} as const;

const STATUS_CFG: Record<string, { label: string; bg: string; text: string; ring: string }> = {
  accepted:    { label: "Accepted",    bg: "rgba(16,185,129,0.12)",  text: "#10b981", ring: "rgba(16,185,129,0.2)"  },
  picked:      { label: "Picked Up",   bg: "rgba(249,115,22,0.12)",  text: "#f97316", ring: "rgba(249,115,22,0.2)"  },
  shopping:    { label: "Shopping",    bg: "rgba(234,179,8,0.12)",   text: "#eab308", ring: "rgba(234,179,8,0.2)"   },
  on_the_way:  { label: "On The Way",  bg: "rgba(168,85,247,0.12)",  text: "#a855f7", ring: "rgba(168,85,247,0.2)"  },
  at_customer: { label: "At Customer", bg: "rgba(99,102,241,0.12)",  text: "#6366f1", ring: "rgba(99,102,241,0.2)"  },
  delivered:   { label: "Delivered",   bg: "rgba(34,197,94,0.12)",   text: "#22c55e", ring: "rgba(34,197,94,0.2)"   },
};
...
export function BatchCardMobile({ order, currentTime }: BatchCardMobileProps) {
  ...
  const typeCfg =
    ORDER_TYPE_CFG[(order.orderType as keyof typeof ORDER_TYPE_CFG) ?? "regular"] ??
    ORDER_TYPE_CFG.regular;

  const statusCfg = STATUS_CFG[order.status] ?? {
    label: order.status.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
  ...
  const deliveryInfo = (() => {
    if (!order.deliveryTime) {
      return order.orderType === "business"
        ? { label: "Within 2 hours", urgent: false, overdue: false, neutral: true }
        : { label: "—", urgent: false, overdue: false, neutral: true };
    }
    const diff = new Date(order.deliveryTime).getTime() - currentTime.getTime();
    const mins = Math.floor(diff / 60_000);
    if (diff < 0) return { label: `${relativeTime(diff)} late`, urgent: false, overdue: true, neutral: false };
    if (mins <= 10) return { label: `${relativeTime(diff)} left`, urgent: true, overdue: false, neutral: false };
    return { label: `in ${relativeTime(diff)}`, urgent: false, overdue: false, neutral: true };
  })();
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="mb-3 grid grid-cols-2 gap-3">
          ...
          >
            <p className="mb-0.5 text-[10px] font-medium uppercase tracking-wider" style={{ color: labelColor }}>
              Earnings
            </p>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          >
            <span className="h-1.5 w-1.5 rounded-full" style={{ background: statusCfg.text }} />
            {statusCfg.label}
          </span>
