      ...
      <Modal.Body>
        <div className="space-y-4 p-2">
            ...
            <div className="flex justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-gray-600">
                  Status: <span className="font-medium text-green-600">{invoiceData.status}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h3 className="text-sm font-medium text-gray-600">Date Created</h3>
              <p className="text-sm">{invoiceData.dateCreated}</p>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-600">Date Completed</h3>
          ...
          {/* Items Table */}
          <div className="mt-4">
            <h3 className="mb-2 text-sm font-medium text-gray-600">Items Found</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="border-b bg-gray-50 text-left">
                  <tr>
                    <th className="p-2 text-sm font-medium text-gray-600">Item</th>
                    <th className="p-2 text-sm font-medium text-gray-600">Qty</th>
                    <th className="p-2 text-sm font-medium text-gray-600">Unit Price</th>
                    <th className="p-2 text-sm font-medium text-gray-600 text-right">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {invoiceData.items.map((item, index) => (
                    <tr key={index} className="border-b">
                      ...
                      <td className="p-2 text-sm">{formatCurrency(item.unitPrice)}</td>
          ...
          <div className="mt-4 space-y-1 rounded-lg bg-gray-50 p-3">
            <div className="flex justify-between">
              ...
              <span className="text-sm">{formatCurrency(invoiceData.subtotal)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-600">Service Fee</span>
              <span className="text-sm">{formatCurrency(invoiceData.serviceFee)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-600">Delivery Fee</span>
              <span className="text-sm">{formatCurrency(invoiceData.deliveryFee)}</span>
            </div>
            <Divider />
            <div className="flex justify-between">
          ...
          <div className="mt-4 rounded-md bg-blue-50 p-3 text-sm text-blue-800">
            <p>
              <strong>Note:</strong> Service fee and delivery fee were added to your available wallet
              balance when you started shopping. The payment reflects only the value of found items.
            </p>
          </div>
        </div>
      </Modal.Body>
