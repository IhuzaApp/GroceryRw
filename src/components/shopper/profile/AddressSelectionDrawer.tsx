  ...
  useEffect(() => {
    if (isLoaded && !autocompleteServiceRef.current) {
      try {
        ...
        logger.error(
          "Error initializing Google Maps AutocompleteService",
          errorMessage
        );
      }
    }
            ...
            <div className="space-y-4">
              <div>
                ...
                {suggestions.length > 0 && (
                  <div
                    className={`mt-1 max-h-32 overflow-y-auto rounded-md border sm:max-h-48 ${
                      theme === "dark"
          ...
          >
            <div className="flex flex-col space-y-2 sm:flex-row sm:justify-end sm:space-x-3 sm:space-y-0">
              ...
              <button
                onClick={handleSave}
                disabled={loading}
                className="w-full rounded-md bg-green-500 px-4 py-2 text-sm font-medium text-white hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 sm:w-auto"
              >
