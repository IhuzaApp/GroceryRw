  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[var(--bg-primary)] transition-colors duration-500 md:ml-16">
      ...
      <div className="pointer-events-none absolute inset-0 z-0">
        ...
        <div className="absolute -right-[10%] top-[20%] h-[50%] w-[50%] animate-pulse rounded-full bg-emerald-500/10 blur-[120px] dark:bg-emerald-400/5" style={{ animationDelay: '2s' }}></div>
        ...
        <div className="mb-24 flex flex-col gap-12 lg:flex-row lg:items-center">
          <div className="max-w-3xl flex-1 space-y-8">
            <div className="inline-flex items-center gap-3 rounded-full border border-green-500/20 bg-green-500/5 px-4 py-2 text-sm font-bold text-green-600 backdrop-blur-md dark:border-green-400/20 dark:bg-green-400/10 dark:text-green-400">
              ...
              <span className="tracking-wide uppercase">The Future of Rwandan Trade</span>
            </div>
            ...
            <div className="flex flex-wrap gap-6 pt-6">
               ...
               >
                 <span className="relative z-10" style={{ color: "#ffffff" }}>Join the Ecosystem</span>
                 <ArrowRight className="relative z-10 h-5 w-5 transition-transform group-hover:translate-x-1" style={{ color: "#ffffff" }} />
               ...
               <div className="flex items-center gap-4 lg:ml-4">
                 <div className="flex -space-x-3">
                   {[1,2,3,4].map(i => (
                     <div key={i} className="h-10 w-10 rounded-full border-2 border-[var(--bg-primary)] bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800"></div>
                   ))}
                 </div>
                 <div className="flex flex-col">
                   <span className="text-sm font-black text-[var(--text-primary)]">+500 Businesses</span>
                   <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] opacity-50">Already Active</span>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="hidden flex-1 lg:block">
            ...
            <div className="grid grid-cols-2 gap-4 opacity-40 grayscale transition-all duration-700 hover:opacity-100 hover:grayscale-0">
               {[
                 { icon: TrendingUp, label: "Market Growth", value: "+24%", color: "text-green-500" },
                 { icon: Globe, label: "Regional Reach", value: "8 Districts", color: "text-blue-500" },
                 { icon: Users, label: "Supply Chain", value: "Locked", color: "text-purple-500" },
                 { icon: Sparkles, label: "Innovation", value: "Top Tier", color: "text-amber-500" }
               ].map((stat, i) => (
                 <div key={i} className="rounded-3xl border border-[var(--bg-secondary)] bg-[var(--bg-primary)]/50 p-6 backdrop-blur-xl transition-all hover:-translate-y-2 hover:border-green-500/30">
        ...
        <div className="space-y-12">
          <div className="flex flex-col items-end justify-between gap-6 md:flex-row md:items-center">
            <div className="space-y-2">
              <h3 className="text-4xl font-black tracking-tight text-[var(--text-primary)]">Market Explorer</h3>
              <p className="font-medium text-[var(--text-secondary)] opacity-60">Real-time pulses from the Rwandan business ecosystem.</p>
            </div>
            
            {/* Sliding Segmented Control */}
            <div className="relative flex w-full items-center rounded-2xl border border-[var(--bg-secondary)] bg-[var(--bg-secondary)]/30 p-1 md:w-auto">
              {/* Sliding Highlight */}
              <div 
                className="absolute inset-y-1 left-1 rounded-xl bg-[var(--bg-primary)] shadow-lg transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
                style={{ 
                  width: 'calc(50% - 4px)',
                  transform: activeTab === 'services' ? 'translateX(0)' : 'translateX(100%)'
                }}
              ></div>
              ...
              <button
                ...
                className={`relative z-10 flex flex-1 items-center justify-center gap-3 px-8 py-3 text-sm font-black transition-colors duration-500 md:flex-none ${
                    activeTab === "services" ? "text-green-600" : "text-[var(--text-secondary)]"
                  }`}
              >
                <Package className={`h-4 w-4 transition-transform duration-500 ${activeTab === 'services' ? 'scale-110' : ''}`} />
                Services
              </button>
              <button
                onClick={() => setActiveTab("rfqs")}
                className={`relative z-10 flex flex-1 items-center justify-center gap-3 px-8 py-3 text-sm font-black transition-colors duration-500 md:flex-none ${
                    activeTab === "rfqs" ? "text-green-600" : "text-[var(--text-secondary)]"
                  }`}
              >
                <FileText className={`h-4 w-4 transition-transform duration-500 ${activeTab === 'rfqs' ? 'scale-110' : ''}`} />
                RFQs
              </button>
            </div>
          </div>
          ...
          <div className="group/marketplace relative overflow-hidden rounded-[2.5rem] border border-[var(--bg-secondary)] bg-[var(--bg-primary)] p-4 shadow-2xl lg:p-6">
            ...
            <div className="absolute inset-0 pointer-events-none z-10 rounded-[2.5rem] shadow-[inset_0_0_80px_rgba(0,0,0,0.02)] dark:shadow-[inset_0_0_80px_rgba(154,154,154,0.01)] transition-opacity group-hover/marketplace:opacity-50"></div>
            
            <div className="max-h-[900px] overflow-y-auto overflow-x-hidden rounded-[2rem] scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-700">
              {activeTab === "services" && (
