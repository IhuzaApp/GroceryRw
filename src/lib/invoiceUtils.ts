  ...
  try {
    ...
    const addWatermark = () => {
      ...
      doc.text(text, centerX, centerY, { angle: -45 });
       ...
       doc.text(invoiceData.invoiceNumber, centerX - textWidth/2, centerY + 60, { angle: -45 });
    };
    ...
    doc.text(`Order #: ${invoiceData.orderNumber}`, pageWidth - margin - 50, yPos);
    
    yPos += 8;
    
    // Add dates
    doc.text(`Date Created: ${invoiceData.dateCreated}`, margin, yPos);
    doc.text(`Date Completed: ${invoiceData.dateCompleted}`, pageWidth - margin - 70, yPos);
    ...
    doc.text(formatCurrency(invoiceData.subtotal), pageWidth - margin - 10, yPos, { align: 'right' });
    
    yPos += 6;
    
    // Service Fee
    doc.text('Service Fee:', margin + 10, yPos);
    doc.text(formatCurrency(invoiceData.serviceFee), pageWidth - margin - 10, yPos, { align: 'right' });
    
    yPos += 6;
    
    // Delivery Fee
    doc.text('Delivery Fee:', margin + 10, yPos);
    doc.text(formatCurrency(invoiceData.deliveryFee), pageWidth - margin - 10, yPos, { align: 'right' });
    ...
    doc.text(formatCurrency(invoiceData.total), pageWidth - margin - 10, yPos, { align: 'right' });
    ...
    yPos += 20;
     ...
     doc.text('Note: Service fee and delivery fee were added to the shopper\'s available wallet balance.', margin, yPos);
     yPos += 4;
