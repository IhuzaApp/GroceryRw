...
interface ChatContextType {
  ...
  openChat: (orderId: string, customerId: string, customerName: string, customerAvatar?: string) => void;
  closeChat: () => void;
...
const convertFirebaseMessage = (fbMessage: FirebaseChatMessage): ChatMessage => {
  return {
...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [messageListeners, setMessageListeners] = useState<{[key: string]: () => void}>({});
  ...
  const openChat = useCallback(async (
    ...
    try {
      ...
      const existingChatIndex = activeChats.findIndex(chat => chat.orderId === orderId);
      
      if (existingChatIndex === -1) {
        // Get or create the conversation in Firebase
        const shopperId = user.id;
        const conversationId = await createConversation(orderId, customerId, shopperId);
        
        // Set up a listener for messages in this conversation
        if (!messageListeners[conversationId]) {
          const unsubscribe = listenForMessages(conversationId, (fbMessages) => {
            ...
            setActiveChats(prev => {
              ...
              if (chatIndex >= 0) {
                ...
                updatedChats[chatIndex] = {
                  ...updatedChats[chatIndex],
                  messages,
                  unreadCount: fbMessages.filter(msg => !msg.isRead && msg.senderId !== user.id).length,
  ...
  const getMessages = useCallback((orderId: string): ChatMessage[] => {
    const chat = activeChats.find(chat => chat.orderId === orderId);
    return chat?.messages || [];
  }, [activeChats]);

  const hasUnreadMessages = useCallback((orderId: string): boolean => {
    const chat = activeChats.find(chat => chat.orderId === orderId);
    return !!chat && chat.unreadCount > 0;
  }, [activeChats]);

  const markMessagesAsRead = useCallback(async (orderId: string) => {
    if (!user?.id) {
      console.error("User not authenticated");
      return;
    }
    ...
    try {
      ...
      setActiveChats(prev => {
        ...
        if (chatIndex >= 0) {
          ...
          updatedChats[chatIndex].messages = updatedChats[chatIndex].messages.map(msg => {
