...
export const FoodCartProvider: React.FC<{ children: React.ReactNode }> = ({
      ...
      try {
        ...
        if (savedCart) {
          ...
          if (
            ...
            Date.now() - parsedCart.timestamp > FOOD_CART_SESSION_DURATION
          ) {
  ...
  const addItem = (restaurant: any, dish: any, quantity: number = 1) => {
    setRestaurants((prevRestaurants) => {
      const existingRestaurantIndex = prevRestaurants.findIndex(
      ...
      if (existingRestaurantIndex >= 0) {
        ...
        const existingDishIndex = prevRestaurants[
          existingRestaurantIndex
        ].items.findIndex((item) => item.id === dish.id);
        ...
        if (existingDishIndex >= 0) {
          ...
          updatedItems = [
            ...prevRestaurants[existingRestaurantIndex].items,
            newItem,
          ];
        }
