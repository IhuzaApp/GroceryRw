...
const PUBLIC_BASE_URL =
  process.env.PUBLIC_BASE_URL || "https://grocery-rw.vercel.app";
...
async function updateShopperTelegramId(shopperId, telegramId) {
  try {
    ...
    if (result.success) {
      console.log(
        `✅ Database updated: Shopper ${shopperId} -> Telegram ${telegramId}`
      );
      return true;
    } else {
...
function createEarningsChart(orders) {
  ...
  const maxEarnings = Math.max(
    ...Object.values(hourlyData).map((d) => d.earnings)
  );

  for (let hour = 0; hour < 24; hour++) {
    const data = hourlyData[hour] || { count: 0, earnings: 0 };
    const barLength =
      maxEarnings > 0 ? Math.round((data.earnings / maxEarnings) * 10) : 0;
    const bar = "█".repeat(barLength) + "░".repeat(10 - barLength);

    chart += `${hour
      .toString()
      .padStart(2, "0")}:00 ${bar} $${data.earnings.toFixed(2)} (${
      data.count
    } orders)\n`;
  }
...
function createWeeklyChart(dailyData) {
  ...
  const maxEarnings = Math.max(
    ...Object.values(dailyData).map((d) => d.earnings)
  );
  ...
  days.forEach((day) => {
    ...
    chart += `${day} ${bar} $${data.earnings.toFixed(2)} (${
      data.count
    } orders)\n`;
  });
...
function createMonthlyChart(weeklyData) {
  ...
  const maxEarnings = Math.max(
    ...Object.values(weeklyData).map((d) => d.earnings)
  );
  ...
  Object.keys(weeklyData).forEach((week) => {
    ...
    chart += `${week} ${bar} $${data.earnings.toFixed(2)} (${
      data.count
    } orders)\n`;
  });
...
bot.onText(/\/start (.+)/, async (msg, match) => {
  ...
  const dbUpdateSuccess = await updateShopperTelegramId(
    shopperId,
    chatId.toString()
  );
  ...
  if (shopper) {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/telegram/today-earnings`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (data && data.success && data.data) {
        ...
        await bot.sendMessage(
          ...
          `📅 ${name}, here's your today's summary:\n\n` +
            ...
            `📊 Average per Order: $${
              orderCount > 0 ? (totalEarnings / orderCount).toFixed(2) : "0.00"
            }\n\n` +
            `📈 Earnings Chart:\n${chart}\n\n` +
...
bot.onText(/\/week/, async (msg) => {
  ...
  if (shopper) {
    try {
      ...
      // Fetch week's data
      const response = await fetch(
        `${API_BASE_URL}/api/telegram/week-earnings`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
bot.onText(/\/month/, async (msg) => {
  ...
  if (shopper) {
    try {
      ...
      // Fetch month's data
      const response = await fetch(
        `${API_BASE_URL}/api/telegram/month-earnings`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
bot.onText(/\/orders/, async (msg) => {
  ...
  if (shopper) {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/telegram/available-orders`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
bot.onText(/\/batches/, async (msg) => {
  ...
  if (shopper) {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/telegram/assigned-batches`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          if (order.deliveryTime) {
            ...
            if (order.minutesRemaining !== null) {
              if (order.minutesRemaining > 0) {
                ...
                message += `   ⚠️ Overdue: ${Math.abs(
                  order.minutesRemaining
                )} min\n`;
              }
            }
          }
